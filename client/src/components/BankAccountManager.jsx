...
/**
 * User Bank Account Manager Component
 * Allows users to manage multiple bank accounts
 */
const BankAccountManager = () => {
  ...
  useEffect(() => {
    ...
    socket.on("bankAccountsUpdated", handleBankAccountsUpdated);

    return () => {
      socket.off("bankAccountsUpdated", handleBankAccountsUpdated);
    };
  }, []);
  ...
  const handleSaveAccount = async () => {
    ...
    // Validate IFSC code format (typically 11 characters)
    ...
    try {
      ...
      if (editingAccount) {
        ...
        response = await bankAccountService.updateBankAccount(editingAccount._id, formData);
      } else {
        // Add new account
  ...
  const handleSetPrimary = async (accountId) => {
    try {
      const response = await bankAccountService.setPrimaryAccount(accountId);
      if (response.success) {
        fetchAccounts();
      } else {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full max-h-96 overflow-y-auto">
            ...
            <div className="space-y-3">
              {/* Account Number */}
              <div>
                <label htmlFor="user-ba-account-number" className="block font-semibold text-sm mb-1">
                  Account Number <span className="text-red-500">*</span>
                </label>
                <input
                  id="user-ba-account-number"
                  type="text"
                  name="accountNumber"
              ...
              {/* IFSC Code */}
              <div>
                <label htmlFor="user-ba-ifsc" className="block font-semibold text-sm mb-1">
              ...
              {/* Account Holder Name */}
              <div>
                <label htmlFor="user-ba-holder" className="block font-semibold text-sm mb-1">
              ...
              {/* Bank Name */}
              <div>
                <label htmlFor="user-ba-bank" className="block font-semibold text-sm mb-1">
              ...
              {/* Account Type */}
              <div>
                <label htmlFor="user-ba-account-type" className="block font-semibold text-sm mb-1">Account Type</label>
                <select
                  id="user-ba-account-type"
                  name="accountType"
              ...
              {!editingAccount && (
                <div className="flex items-center">
                  ...
                  />
                  <label htmlFor="user-ba-primary" className="ml-2 font-semibold text-sm">Set as primary account</label>
                </div>
              )}
            </div>
